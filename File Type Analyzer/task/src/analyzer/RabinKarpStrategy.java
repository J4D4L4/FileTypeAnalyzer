...
public class RabinKarpStrategy implements AnalyseStrategy{
    @Override
    public String analyse(String path, String pattern, String resultString) {
        LocalDateTime startTime = LocalDateTime.now();
        boolean foundPattern = RabinKarpAlgo.search(pattern, FileReaderLocal.readFile(path));
    ...
    public String analyse(String path, FilePattern[] patterns, String resultString) {
        ...
        for (FilePattern pattern : patterns){
            if(RabinKarpAlgo.search(pattern.pattern, FileReaderLocal.readFile(path))){
